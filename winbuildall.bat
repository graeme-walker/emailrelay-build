...
if "%1"=="download" (
	...
	if not exist emailrelay-src (
		...
		curl -L -O https://sf.net/projects/emailrelay/files/emailrelay/2.6.1/emailrelay-2.7rc1-src.tar.gz
		mkdir emailrelay-src
		tar -m -C emailrelay-src --strip-components=1 emailrelay-2.6.1-src.tar.gz
	)
	goto end
)
