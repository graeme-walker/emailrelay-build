...
{
	my $fh = new FileHandle( "winbuild.cfg" , "w" ) or die "error: cannot create winbuild.cfg\n" ;
	map { print $fh $_ , "\n" } keys %cfg_options ;
	$fh->close() or die ;
}
...
sub run_cmake
{
	...
	for my $arch_args ( @arch_args )
	{
		...
		print "cmake($arch): running: [",join("][",$cfg_path_cmake,@args),"]\n" ;
		$rc = system( $cfg_path_cmake , @args ) ;
		last if $rc == 0 ;

		$i++ ;
		my $final = ( $i == scalar(@arch_args) ) ;
		print "cmake($arch): cannot use that cmake generator: trying another\n" if !$final ;
	}
	print "cmake-exit=[$rc]\n" ;
...
sub run_build
{
	...
	push @artifacts , "$arch/src/main/$confname/emailrelay-gui.exe" if $cfg_with_gui ;

	print "build($arch,$confname): running: [",join("][",$cfg_path_cmake,@args),"]\n" ;
	my $rc = system( $cfg_path_cmake , @args ) ;
	print "build($arch,$confname): exit=[$rc]\n" ;
	die unless $rc == 0 ;

	for my $exe ( @artifacts )
	{
		print "build($arch,$confname): " ,
...
sub run_mbedtls_cmake
{
	...
	{
		print "mbedtls-cmake($arch): already got [$arch/mbed tls.sln]: not running cmake\n" ;
	}
	else
	{
		my @runtime_args = () ;
		...
		for my $arch_args ( @arch_args )
		{
			...
			print "mbedtls-cmake($arch): running: [",join("][",$cfg_path_cmake,@args),"]\n" ;
			$rc = system( $cfg_path_cmake , @args ) ;
...
sub run_mbedtls_build
{
	...
	print "mbedtls-build($arch,$confname): running: [",join("][",$cfg_path_cmake,@args),"]\n" ;
	my $rc = system( $cfg_path_cmake , @args ) ;
	print "mbedtls-build($arch,$confname): exit=[$rc]\n" ;
	die unless $rc == 0 ;
}
