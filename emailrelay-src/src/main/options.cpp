...
G::Options Main::Options::spec()
{
	...
	G::Options::add( opt , 'l' , "log" ,
		#ifdef G_WINDOWS
		tx("log information on stderr and to the system event log! "
		...
		t_logging ) ;
			...
			// standard error stream and the system log separately. Note that --as-server,
	...
	G::Options::add( opt , 't' , "no-daemon" ,
		...
		t_process ) ;
			...
			// terminal.
	#else
	// undocumented but still allowed -- prefer "--show=window"
	G::Options::add( opt , 't' , "no-daemon" ,
		"" , "" ,
		M::zero , "" , 0 ,
		t_process ) ;
	...
	G::Options::add( opt , 'k' , "syslog" ,
		...
		t_logging ) ;
			...
			// as a convenient override when using --as-client.

	G::Options::add( opt , 'n' , "no-syslog" ,
		#ifdef G_WINDOWS
		tx("disables use of the system event log (always overridden by --syslog)") , "" ,
		#else
	...
	G::Options::add( opt , 'H' , "hidden" ,
		...
		M::zero , "" , 30 ,
		t_process ) ;
			// Hides the application window and disables all message boxes,
			// overriding any --show option. This is useful when running
	...
	G::Options::add( opt , '\0' , "show" ,
		...
		t_process ) ;
			//example: window,tray
			// Starts the application window in the given style: "hidden",
	...
	G::Options::add( opt , 'd' , "as-server" ,
		...
		t_basic , t_smtpserver ) ;
			...
			// spools them. Use --log instead of --as-server to keep the standard
			// error stream open.
	...
	G::Options::add( opt , 's' , "spool-dir" ,
		...
		M::one , "dir" , 10 ,
		t_basic ) ;
			//example-unix: /var/spool/emailrelay
			//example-windows: C:/ProgramData/E-MailRelay/spool
	...
	G::Options::add( opt , 's' , "delivery-dir" ,
		...
		M::one , "dir" , 30 ,
		t_smtpserver ) ;
			//example-unix: /var/spool/emailrelay/in
			//example-windows: C:/ProgramData/E-MailRelay/spool/in
	...
	G::Options::add( opt , '\0' , "server-tls-certificate" ,
		...
		M::many , "pem-file" , 30 ,
		t_tls , t_smtpserver ) ;
			//example-unix: /etc/ssl/certs/emailrelay.pem
			//example-windows: C:/ProgramData/E-MailRelay/emailrelay.pem
	...
	G::Options::add( opt , '\0' , "server-tls-verify" ,
		...
		M::one , "ca-list" , 30 ,
		t_tls , t_smtpserver ) ;
			//example-unix: /etc/ssl/certs/ca-certificates.crt
			//example-windows: C:/ProgramData/E-MailRelay/ca-certificates.crt
	...
	G::Options::add( opt , 'C' , "client-auth" ,
		...
		M::one , "file" , 30 ,
		t_auth , t_smtpclient ) ;
			//example-unix: /etc/private/emailrelay.auth
			//example-windows: C:/ProgramData/E-MailRelay/emailrelay.auth
	...
	G::Options::add( opt , 'N' , "log-file" ,
		tx("redirects stderr logging to file! "
			"(with '%d' replaced by the current date)") , "" ,
		M::one , "file" , 30 ,
		t_logging ) ;
			//example-unix: /var/log/emailrelay-%d
			//example-windows: C:/ProgramData/E-MailRelay/log-%d.txt
	...
	G::Options::add( opt , 'Z' , "server-smtp-config" ,
		...
		t_smtpserver ) ;
			...
			// 'smtputf8', 'smtputf8strict', 'nostrictparsing' and 'noalabels'.
	...
	G::Options::add( opt , 'z' , "filter" ,
		...
		t_smtpserver , t_filter ) ;
			//example-unix: /usr/local/sbin/emailrelay-filter
			//example-windows: C:/Program\ Files/E-MailRelay/filter.bat
	...
	G::Options::add( opt , 'D' , "domain" ,
		...
		t_smtpserver ) ;
			//example: smtp.example.com
			//example: xn--bcher-kva.example.com
			// Specifies the domain name that is used in SMTP client EHLO
			// commands, server EHLO responses, "Received" lines, and for
			// generating authentication challenges. The SMTP client will
			// use an IP address in the EHLO command if the given domain
			// is not a dotted FQDN. If this option is not used at all
			// then the default value is the canonical name returned from
			// a DNS query of the local hostname, or the system's FQDN
			// on Windows.
	...
	G::Options::add( opt , 'I' , "interface" ,
		...
		t_smtpserver ) ;
			...
			// unix use a syntax like this: --interface=smtp=fd#3.
	...
	G::Options::add( opt , 'i' , "pid-file" ,
		...
		M::many , "path" , 30 ,
		t_process ) ;
			//example-unix: /run/emailrelay/emailrelay.pid
			//example-windows: C:/ProgramData/E-MailRelay/pid.txt
	...
	G::Options::add( opt , '\0' , "address-verifier" ,
		...
		M::one , "program" , 30 ,
		t_smtpserver ) ;
			//example-unix: /usr/local/sbin/emailrelay-verifier.sh
			//example-windows: C:/ProgramData/E-MailRelay/verifier.js
	...
	G::Options::add( opt , 'Y' , "client-filter" ,
		...
		t_smtpclient , t_filter ) ;
			//example-unix: /usr/local/sbin/emailrelay-client-filter
			//example-windows: C:/ProgramData/E-MailRelay/client-filter.js
			// Runs the specified external filter program whenever a mail message is
	...
	G::Options::add( opt , 'F' , "pop-auth" ,
		...
		t_auth , t_pop|t_smtpserver ) ;
			//example-unix: /etc/private/emailrelay-pop.auth
			//example-windows: C:/ProgramData/E-MailRelay/pop.auth
			//example: pam:
	...
	G::Options::add( opt , 'M' , "size" ,
		tx("limits the size of incoming e-mail message content") , "" ,
		M::one , "bytes" , 30 ,
		t_smtpserver ) ;
			//example: 10000000
	...
	G::Options::add( opt , '\0' , "dnsbl" ,
		tx("configuration for DNSBL blocking of remote SMTP client addresses") , "" ,
		M::many , "config" , 30 ,
		t_smtpserver ) ;
			//example: spam.dnsbl.example.com,block.dnsbl.example.com,1,1000,1.1.1.1:53
			...
			// is made up of comma-separated fields: the list of DNSBL servers,
			// an optional rejection threshold, an optional timeout in
			// milliseconds, and optionally the transport address of the
			// DNS server.
