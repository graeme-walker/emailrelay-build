...
namespace G
{
	namespace nowide
	{
		...
		using statbuf_type = struct ::__stat64 ;
		...
		inline std::string getComputerNameEx( COMPUTER_NAME_FORMAT name_type = ComputerNameNetBIOS )
		{
			...
			if( !GetComputerNameExW( name_type , nullptr , &size ) && GetLastError() == ERROR_MORE_DATA && size )
			...
			bool with_domain , DWORD * domainsize_p , SID_NAME_USE * type_p )
		{
			...
			return LookupAccountNameW( nullptr , Convert::widen(full_name).c_str() , sid_buffer , sidsize_p ,
			...
			bool with_domain , DWORD * domainsize_p , SID_NAME_USE * type_p )
		{
			...
			std::vector<wchar_t> domainbuffer( std::max(DWORD(1),*domainsize_p) ) ;
			BOOL rc = LookupAccountSidW( nullptr , sid ,
		...
		{
			std::wstring wtitle = Convert::widen( title ) ;
			page->pszTitle = wtitle.c_str() ;
			page->pszTemplate = dialog_id ? MAKEINTRESOURCEW(dialog_id) : nullptr ;
		...
		{
			std::wstring wtitle = Convert::widen( title ) ;
			header->pszIcon = icon_id ? MAKEINTRESOURCEW(icon_id) : nullptr ;
		...
		inline void reportEvent( HANDLE h , DWORD id , WORD type , const char * message )
		{
			...
			std::array<const wchar_t*,2U> p = {{ wmessage.c_str() , nullptr }} ;
			ReportEventW( h , type , 0 , id , nullptr , 1 , 0 , p.data() , nullptr ) ;
		}
			...
			HKEY hkey_in = HKEY_LOCAL_MACHINE , bool * is_new_p = nullptr )
		{
			...
			result = RegCreateKeyExW( hkey_in , Convert::widen(reg_path.str()).c_str() ,
				0 , nullptr , 0 , KEY_ALL_ACCESS , nullptr , key_out_p , &disposition ) ;
			if( is_new_p )
		...
		inline LSTATUS regQueryValueType( HKEY key , const Path & sub , DWORD * type_out_p , DWORD * size_out_p )
		{
			return RegQueryValueExW( key , Convert::widen(sub.str()).c_str() ,
				nullptr , type_out_p , nullptr , size_out_p ) ;
		}
		inline LSTATUS regGetValueString( HKEY key , const Path & sub , std::string * value_out_p )
		{
			DWORD type = 0 ;
			DWORD size = 0 ;
			LSTATUS status = RegQueryValueExW( key , Convert::widen(sub.str()).c_str() , nullptr , &type , nullptr , &size ) ;
			if( type != REG_SZ )
				return ERROR_INVALID_DATA ;
			if( status != ERROR_SUCCESS )
				return status ;
			std::vector<wchar_t> buffer( static_cast<std::size_t>(size)+1U , '\0' ) ;
			status = RegQueryValueExW( key , Convert::widen(sub.str()).c_str() , nullptr , &type ,
		...
		inline LSTATUS regGetValueNumber( HKEY key , const Path & sub , DWORD * value_out_p )
		{
			DWORD type = 0 ;
			DWORD size = 4 ;
			LSTATUS status = RegQueryValueExW( key , Convert::widen(sub.str()).c_str() , nullptr ,
		...
		inline BOOL createProcess( std::string_view exe , std::string_view command_line_in ,
		...
		{
			std::wstring command_line = Convert::widen( command_line_in ) ; // mutable
			wchar_t * command_line_p = const_cast<wchar_t*>( command_line.c_str() ) ;
			return CreateProcessW( exe.empty() ? nullptr : Convert::widen(exe).c_str() ,
				command_line.empty() ? nullptr : command_line_p ,
				nullptr , nullptr ,
		...
		inline std::string formatMessage( DWORD e )
		{
			...
			FormatMessageW( FORMAT_MESSAGE_ALLOCATE_BUFFER |
				...
				reinterpret_cast<wchar_t*>(&ptr) , 1 , nullptr ) ;
		...
		{
			std::vector<wchar_t> buffer ;
			std::array<std::size_t,3U> sizes = {{ 80U , 1024U , 32768U }} ; // documented limit of 32k
			for( auto size : sizes )
			{
				buffer.resize( size+1U , '\0' ) ;
		...
		inline int chdir( const Path & dir )
		{
			return _wchdir( Convert::widen(dir.str()).c_str() ) ;
		}
		inline Path cwd()
		{
		...
		inline bool winHelp( HWND hwnd , const Path & path , unsigned int id )
		{
			return WinHelpW( hwnd , Convert::widen(path.str()).c_str() , id , 0 ) ;
		}
		...
		inline BOOL startServiceCtrlDispatcher( ServiceMainWFn w_fn ) noexcept
		{
			std::array<SERVICE_TABLE_ENTRYW,2U> table = {{ { const_cast<wchar_t*>(L"") , w_fn } , { nullptr , nullptr } }} ;
			return StartServiceCtrlDispatcherW( table.data() ) ;
		}
		...
		{
			return link_p->SetIconLocation( Convert::widen(icon.str()).c_str() , static_cast<int>(i) ) ;
		}
		...
		inline HRESULT persistFileSave( IPersistFile * persist_file_p , const Path & link_path , BOOL remember )
		{
