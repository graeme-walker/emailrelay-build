...
namespace G
{
	namespace Base64Imp
	{
		...
		static constexpr std::string_view character_map_with_pad( "=ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/" , 1+26+26+10+2 ) ;
		static constexpr std::string_view character_map( character_map_with_pad.data()+1 , character_map_with_pad.size()-1 ) ;
		static constexpr char pad = '=' ;

		using iterator_in = std::string_view::const_iterator ;
		using iterator_out = std::back_insert_iterator<std::string> ;

		std::string encode( std::string_view , std::string_view eol ) ;
		std::string decode( std::string_view , bool do_throw , bool strict ) ;
		bool valid( std::string_view , bool strict ) ;

		void encode_imp( iterator_out , std::string_view , std::string_view , std::size_t ) ;
		void decode_imp( iterator_out , std::string_view s , bool & error ) ;
		...
		bool strictlyValid( std::string_view ) noexcept ;
...
std::string G::Base64::encode( std::string_view s , std::string_view eol )
{
	return Base64Imp::encode( s , eol ) ;
}

std::string G::Base64::decode( std::string_view s , bool do_throw , bool strict )
{
	return Base64Imp::decode( s , do_throw , strict ) ;
}

bool G::Base64::valid( std::string_view s , bool strict )
{
...
std::string G::Base64Imp::encode( std::string_view input , std::string_view eol )
{
	std::string result ;
...
void G::Base64Imp::encode_imp( iterator_out result_p , std::string_view input , std::string_view eol , std::size_t blocks_per_line )
{
...
std::string G::Base64Imp::decode( std::string_view input , bool do_throw , bool strict )
{
	bool error = false ;
...
void G::Base64Imp::decode_imp( iterator_out result_p , std::string_view s , bool & error )
{
...
bool G::Base64Imp::valid( std::string_view input , bool strict )
{
	if( strict && !strictlyValid(input) )
		return false ;
...
bool G::Base64Imp::strictlyValid( std::string_view s ) noexcept
{
	if( s.empty() )
		return true ;
