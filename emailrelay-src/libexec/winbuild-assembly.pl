...
#
# Copies files into a Windows release assembly ready for zipping and
# distribution. Called from "winbuild.bat" and "winbuildall.bat".
#
...
GetOptions( \%opt ,
	...
	"version=s" , "arch=s" , "config=s" , "static" , "no-keygen" , "winxp" )
	or _die( "usage error" ) ;
...
sub install
{
	...
	if( $static )
	{
		# no windeployqt, just copy qtcore translation files
		copy_files( "$qt_build_dir/qttranslations/translations/qt_*.qm" , "$dst_dir/translations/" , {not_match=>qr/qt_help_/,at_least=>10} ) ;
		copy_files( "$qt_build_dir/qttranslations/translations/qt_*.qm" , "$dst_dir/payload/files/gui/translations/" , {not_match=>qr/qt_help_/,at_least=>10} ) ;
	}
	else
	{
		# use windeployqt
	...
	copy_files( "$src_dir/src/gui/*.qm" , "$dst_dir/translations/" ) ;
	copy_files( "$src_dir/src/gui/*.qm" , "$dst_dir/payload/files/gui/translations/" ) ;
...
sub install_winxp
{
	...
	{
		my $fh = new IO::File( "$dst_dir/config.txt" , "w" ) or die ;
		print $fh "#\r\n# config.txt\r\n#\r\n\r\n" ;
	...
	{
		my $fh = new IO::File( "$dst_dir/emailrelay-service.cfg" , "w" ) or die ;
...
sub install_core
{
	...
	if( _old($version) ) { $examples = "bin" }

	my $txt = $is_payload ? "txt" : "." ;
	my %copy = qw(
		__src__/README __txt__/readme.txt
		__src__/COPYING __txt__/copying.txt
		__src__/AUTHORS __txt__/authors.txt
		__src__/LICENSE __txt__/license.txt
		__src__/NEWS __txt__/news.txt
		__src__/ChangeLog __txt__/changelog.txt
		...
		__src__/bin/emailrelay-service-install.js programs/
		__src__/__examples__/emailrelay-* examples/
		__src__/doc/emailrelay.css doc/
		...
		__src__/doc/*.html doc/
		__src__/doc/developer.txt doc/
		__src__/doc/reference.txt doc/
		__src__/doc/userguide.txt doc/
		__src__/doc/windows.txt doc/
		__src__/doc/windows.txt __txt__/readme-windows.txt
		__src__/doc/doxygen-missing.html doc/doxygen/index.html
	) ;
	while( my ($from,$to) = each %copy )
	{
...
sub copy_files
{
	...
	_die( "too few matching files [$glob]" ) if scalar(@files) < $opt->{at_least} ;
