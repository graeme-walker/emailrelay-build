...
sub read_makefiles
{
	...
	else
	{
		...
		%vars = $cs->vars() ;
		$vars{top_srcdir} = sub { AutoMakeParser::up_to_base($_[0]) } ;
		$vars{top_builddir} = sub { AutoMakeParser::up_to_base($_[0]) } ;
	}
	...
	for my $m ( @m )
	{
		...
		if( $opt_for_windows )
		{
			...
			push @qt6_static_sys_libnames , (
				...
				"dwmapi" , "dwrite" , "dxgi" , "dxguid" ,
				"d2d1" , "d3d11" , "d3d12" , "imm32" , "netapi32" , "ole32" ,
				"oleaut32" , "shlwapi" , "userenv" , "uxtheme" , "version" ,
				"winmm" , "winspool" , "wtsapi32" , "gdi32" ,
			) ;
		}
		...
		if( $opt_for_windows || $m->{e_is_gui_dir} )
		{
			# no G_LIB_SMALL code-size optimisation on windows or for unix gui
		...
		for my $program ( @programs )
		{
			...
			my ( $mcfile , $rcfile , $binfile , $manifest ) ;
			if( $mkey eq "emailrelay" )
			{
				$mcfile = "messages.mc" ;
				$rcfile = "emailrelay.rc" ;
				$binfile = "MSG00001.bin" ;
				$manifest = "emailrelay.exe.manifest" ; # also use linker options for uac and dependencies
			}
			elsif( $mkey eq "emailrelay-gui" )
			{
				$rcfile = "emailrelay-gui.rc" ;
				$manifest = "emailrelay-gui.exe.manifest" ;
			}
			elsif( $mkey eq "emailrelay-service" )
			{
				$manifest = "emailrelay-service.exe.manifest" ;
			}
			...
			my $uses_commoncontrols = $opt_for_windows && ( $program eq "emailrelay" ) ;
			...
			$m->{e_program}->{$program}->{uses_commoncontrols} = $uses_commoncontrols ;
			$m->{e_program}->{$program}->{commoncontrols_option} = $commoncontrols_option ;
		}
	}
	return @m ;
}
...
{
	return (
		top_srcdir => sub { AutoMakeParser::up_to_base($_[0]) } ,
		top_builddir => sub { AutoMakeParser::up_to_base($_[0]) } ,
		sbindir => "." ,
		mandir => "." ,
		localedir => "." ,
